...
def KLP_Student_View(request, student_id):
	...
	resp=KLP_Student(queryset = Student.objects.filter(pk=0), permitted_methods = ('GET', 'POST'), responder = TemplateResponder(template_dir = 'viewtemplates', template_object_name = 'student',),)(request, student_id, **kwrg)
...
def KLP_Student_Update(request, student_id, counter=0):
	...
	buttonType = request.POST.get('form-buttonType')
        #before Child.objects.all()    
	KLP_Edit_Student =KLP_Student(queryset = Child.objects.filter(pk=0), permitted_methods = ('GET', 'POST'), responder = TemplateResponder(template_dir = 'edittemplates', template_object_name = 'child',extra_context={'buttonType':buttonType,'modelName':"student", 'counter':counter} ), receiver = XMLReceiver(),)
