...
from Akshara.schools.models import current_academic
...
def KLP_Child_View(request, child_id):
	...
	kwrg = {'is_entry':True}
        #before Child.objects.all()
	resp=KLP_Child(queryset = Child.objects.filter(pk=0), permitted_methods = ('GET', 'POST'), responder = TemplateResponder(template_dir = 'viewtemplates', template_object_name = 'child',),)(request, child_id, **kwrg)
...
def KLP_Child_Update(request, child_id):
	...
	referKey = request.POST.get('form-0-boundary')
        #before Child.objects.all()
	KLP_Edit_Child =KLP_Child(queryset = Child.objects.filter(pk=0), permitted_methods = ('GET', 'POST'), responder = TemplateResponder(template_dir = 'edittemplates', template_object_name = 'child',extra_context={'buttonType':buttonType, 'referKey':referKey} ), receiver = XMLReceiver(),)
...
def getStudentSearch(request, boundary, fieldName,searchtext):
	...
	if fieldName=='dob':
		...
		child_list = Child.objects.filter(dob =searchtext, boundary__id = boundary).values_list('id',flat=True)

	if fieldName=='gender':
		child_list = Child.objects.filter(gender__startswith=searchtext, boundary__id = boundary).values_list('id',flat=True)

	if fieldName=='mt':
		child_list = Child.objects.filter(mt__name__startswith=searchtext, boundary__id = boundary).values_list('id',flat=True)

	if fieldName=='mother':
		child_list = Child.objects.filter(relations__relation_type="Mother",relations__name__startswith=searchtext, boundary__id = boundary).values_list('id',flat=True)
...
class StdGrpFilter(Resource):    
    ...
    def read(self,request,school_id):     
         if 1:     
            stdgrp_list = StudentGroup.objects.filter(content_type__model = "school", object_id=school_id,group_type="Class")
            respStr = ''
            for stdgrp in stdgrp_list:
                respStr += '%s$$%s %s&&' %(stdgrp.id, stdgrp.name,stdgrp.section)
...
def childsql(request,boundary_id):
	...
	if children_id and studentgroup:
		...
		for child_id in children_id:
			...
			studentparam={'id':None,'child':child,'school':school,'active':2}
			stdobj=Student.objects.get_or_create(**studentparam)

			param={'id':None,'student':stdobj[0],'student_group':studentgroup,'academic':academic,'active':2}
		...
		for student_id in students_id:
			...
			stdgrp_rels = Student_StudentGroupRelation.objects.get_or_create(**param)
			count = count+1
	return HttpResponseRedirect('/boundary/'+str(boundary_id)+'/child/view/?count='+str(count))

urlpatterns = patterns('',    
   url(r'^boundary/(?P<referKey>.*)/child/creator/$', KLP_Child_Create),
