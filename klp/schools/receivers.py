""" This file containd the methods  and receiver methods
to check the user permissions and to assign permission on
...
if (userObj.id is not None or userObj.is_active):
    ...
    #assessment object
    chkPerm = False
    userAsmList = UserAssessmentPermissions.objects.filter
    (user=userObj, instituion=instObj, assessment=assessmentObj).values()
    if userAsmList:
    ...
    if not(userObj.is_superuser or userObj.is_staff or chkPerm):
        # If user is not super user and he is not in staff and
        #user doesn't has permission with intitution object raise
        #Insufficient Previliges exception
        raise Exception("Insufficient Previliges")


def KLP_user_Perm(userObj, modelName, operation):
    """ This method is used to check user operational permissions
    based on model """
    """ Get User, model name and operation(Add/update/delete) to
    check permissions"""
    # get user groups
    ...
    if userObj.is_active:
        if userObj.is_superuser:
            ...
            # if user in admin group allow to add/update/delete
            #users and pemissions
            if modelName != 'Users':
                raise Exception("Insufficient Previliges")
        elif userObj.is_staff:
            # if user is in staff allow to access all models
            #other than users
            if modelName == 'Users':
                raise Exception("Insufficient Previliges")
        elif 'Data Entry Executive' in user_GroupsList:
            # if user in Data Entry Executive group allow to access
            #'Institution', 'Staff', 'StudentGroup', 'Student'
            #and 'Answer' models
            if modelName.lower() not in ['institution', 'staff',
            'studentgroup', 'student', 'answer']:
                raise Exception("Insufficient Previliges")
        elif 'Data Entry Operator' in user_GroupsList:
            # if user in Data Entry Operator group allow to access
            #'Student' and 'Answer' models
            if modelName.lower() not in ['student', 'answer']:
                raise Exception("Insufficient Previliges")
        else:
            # if user not in any of the group and not staff and not
            #super user then raise Insufficient Previliges exception
            raise Exception("Insufficient Previliges")
    else:
...
def KLP_NewInst_Permission(sender, instance, created, **kwargs):
    """ This receiver method is used to assign permissions
    to users on new institution creation"""
    ...
    if created:
        ...
        active=2)
        # get all active users in Data Entry Executive and Data
        #Entry Operator group
        users_List = User.objects.filter(groups__name__in=[
        'Data Entry Executive', 'Data Entry Operator'], is_active=1)
        ...
        for user in users_List:
            ...
            if lenTrue == lenInst - 1:
                ...
                #boundary except newly created institution, set
                #permissions to user for new institution also
