...
class Institution_address(models.Model):
    ...
    null=True, help_text="Can be comma separated")
    routeInformation = models.CharField(max_length = 500,
    blank=True, null=True, help_text="Can be comma separated")
# Register model for to store information in fullhistory
...
class Boundary(models.Model):
    ...
    boundary_category = models.ForeignKey(Boundary_Category, blank=True,
    null=True)
    ...
    def getPermissionChild(self, boundaryType):
        if Boundary.objects.filter(parent__id=self.id,
    ...
    def getPermissionViewUrl(self):
        ...
        title="boundary"/> % s</a>' % (self.id, self.name, self.name)
...
class Institution(models.Model):
    ...
    institution_gender = models.CharField(max_length=10, choices=
    Institution_Gender, default="co-ed")
    ...
    def getChild(self):
        if StudentGroup.objects.filter(institution__id =
        self.id, active=2).count():
            return True
        else:
            return False
    ...
    def getViewUrl(self):
        return '<a href="/institution/%s/view/" onclick =
        "return KLP_View(this)" class="KLP_treetxt" title="%s">
    ...
    def CreateNewFolder(self):
        retStr = '<span><a href="/institution/%s/view/" onclick =
        "return KLP_View(this)" class="KLP_treetxt" title="%s">
        <img src="/static_media/tree-images/reicons/institution.gif"
        ...
        if groupObjects:
            ...
            for groupObj in groupObjects:
                ...
                if groupName == '0':
                    ...
                    retStr = retStr + """<li id="%s"><span>
                    <a class="KLP_treetxt"
                    onclick="return KLP_View(this)" href="/
                    studentgroup/%s/view/"
...
class StudentGroup(models.Model):
    ''' Here it holds the informaion of the class and
    ...
    def getStudentProgrammeUrl(self, filter_id, secfilter_id):
        ...
        if sec == None:
            sec = ''
        return '<a href="/studentgroup/%s/programme/%s/assessment/%s/view"
        ...
        '<span style="color:green;font-size:12px">%s</span></a>'
        % (self.id, filter_id, secfilter_id, groupName, sec,
    ...
    def CreateNewFolder(self):
        ...
        if groupName == '0':
            groupName = 'Anganwadi Class'
        return '<span><a href="/studentgroup/%s/view/" onclick =
...
class Staff(models.Model):
    ...
    def getAssigendClasses(self):
        return 	StudentGroup.objects.filter(staff_studentgrouprelation__
        staff__id = self.id, staff_studentgrouprelation__active=2)
# Register model for to store information in fullhistory
...
class Student(models.Model):
    ...
    def getViewUrl(self):
        ...
        "return KLP_View(this)" class="KLP_treetxt" title="%s"> %s </a>'
    ...
    def CreateNewFolder(self):
        return '<span><img src="/static_media/tree-images
        '/reicons/student.gif" title="student" />&nbsp<a href="
        '/boundary/%s/institution/%s/classes/%s/sections/%s/
        'students/%s/view/" onclick="return KLP_View(this)"
        'class="KLP_treetxt"> %s </a><a href="/boundary/%s
        '/institution/%s/classes/%s/sections/%s/students/%s/edit/"
        'onclick="return KLP_View(this)">
        '<img src="/static_media/images/pagebuilder_edit.gif"
        title="Edit"/></a><span class="delConf" onclick=
        "deleteSchool(\'%s\', \'student\', \'%s\')"><img width="11"
        title="Delete" src="/static_media/images/PageRow_delete.gif"
        title="Delete"></span></span>' %(self.class_section.
        'classname.sid.boundary.id, self.class_section.
        'classname.sid.id, self.class_section.
        'classname.id, self.class_section.id,self.id,self.name,
        'self.class_section.
        'classname.sid.boundary.id, self.class_section.
        'classname.sid.id, self.class_section.classname.id,self.
        'class_section.id, self.id, self.id, self.name)
# Register model for to store information in fullhistory
...
class Programme(models.Model):
    ...
    def CreateNewFolder(self):
        return '<span><a href="/programme/%s/view/" onclick=
...
class Assessment(models.Model):
    ...
    def getViewUrl(self):
        return '<a id="assessment_%s_view" href="/assessment/%s/view/"
        'onclick="return KLP_View(this)" class="KLP_treetxt" title="%s">
        '<img src="/static_media/tree-images/reicons/assessment.gif"
        'title="Assessment" /> &nbsp; <span id="assessment_%s_text">%s</span>
        '</a>' %(self.id, self.id, self.name, self.id, self.name)

    def getModuleName(self):
        return 'assessment'

    def CreateNewFolder(self):
        return '<span><a id="assessment_%s_view" href="
        '/assessment/%s/view/" onclick="return KLP_View(this)"
        'class="KLP_treetxt" title="%s"> <img src="
        '/static_media/tree-images/reicons/assessment.gif"
        'title="Assessment" /> &nbsp; <span id="assessment_%s_text">
        '%s</span></a></span>' %(self.id, self.id, self.name,
        self.id, self.name)
# Register model for to store information in fullhistory
...
class Assessment_StudentGroup_Association(models.Model):
    '''This Class stores the Assessment and Student Group
...
class Question(models.Model):
    ...
    def getViewUrl(self):
        ...
        '&nbsp<span id="question_%s_text">%s</span> </a>'
...
class Answer(models.Model):
    ...
    blank = True, null = True)
    lastModifiedDate = models.DateField(default=datetime.date.today,
    blank = True, null = True)
    lastmodifiedBy = models.ForeignKey(User, blank = True, null = True,
    related_name = 'lastmodifiedBy')
...
class UserAssessmentPermissions(models.Model):
    ...
    instituion and assessment permissions"""
