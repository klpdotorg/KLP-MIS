...
class Institution(models.Model):
	...
	def getViewUrl(self):
		return '<a href="/institution/%s/view/" onclick="return KLP_View(this)" class="KLP_treetxt" title="%s"> <img src="/static_media/tree-images/reicons/institution.gif" title="institution" /> &nbsp; <span id="institution_%s_text">%s</span> </a>' %(self.id, self.name +' ('+str(self.id)+')', self.id, self.name+' ('+str(self.id)+')')
	...
	def CreateNewFolder(self):
		retStr = '<span><a href="/institution/%s/view/" onclick="return KLP_View(this)" class="KLP_treetxt" title="%s"> <img src="/static_media/tree-images/reicons/institution.gif" title="institution" /> &nbsp; <span id="institution_%s_text">%s</span></a> </span>' %(self.id, self.name +' ('+str(self.id)+')', self.id, self.name+' ('+str(self.id)+')')
		...
		if groupObjects:
			...
			for groupObj in groupObjects:
				...
				retStr = retStr + """<li id="%s"><span><a class="KLP_treetxt" onclick="return KLP_View(this)" href="/studentgroup/%s/view/" title="%s"> <img title="Class" src="/static_media/tree-images/reicons/studentgroup_%s.gif"> <span id="studentgroup_%s_text">%s</span>  </a></span></li>""" %(groupObj.id, groupObj.id, groupName, groupObj.group_type,groupObj.id, groupName)
			retStr = retStr + "</ul>"
		return retStr
...
class StudentGroup(models.Model):
	...
	name = models.CharField(max_length=50)
	section = models.CharField(max_length=10,choices=Alpha_list,blank=True,default="")
	active = models.IntegerField(blank = True, null = True,default=2)
