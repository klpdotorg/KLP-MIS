...
def KLP_Programme_View(request, programme_id):
    ...
    resp = KLP_Programme(queryset=Programme.objects.
    filter(pk=programme_id), permitted_methods=('GET', 'POST'),
    responder = TemplateResponder(template_dir='viewtemplates',
    template_object_name = 'programme',),)(request, programme_id, **kwrg)
...
def KLP_Programme_Create(request):
    ...
    if currentMont > 4:
        endYear = endYear + 1
        #before Programme.objects.all()
        KLP_Create_Programme = KLP_Programme(queryset=
        Programme.objects.filter(pk=0), permitted_methods
        = ('GET', 'POST'), responder=
        TemplateResponder(template_dir='viewtemplates',
        template_object_name = 'programme', extra_context
...
def KLP_Programme_Update(request, programme_id):
    """ To update Selected Programme programme/
    (?P<programme_id>\d+)/update/"""
    ...
    if currentMont > 4:
        endYear = endYear + 1
        #before Programme.objects.all()
        KLP_Edit_Programme = KLP_Programme(queryset=
        Programme.objects.filter(pk=programme_id), permitted_methods
...
class KLP_Get_Programms(Resource):
    """ To get  programmes based on type selected filter
    /(?P<type_id>\d+)/programme/"""
