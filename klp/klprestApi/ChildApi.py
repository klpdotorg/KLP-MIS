...
def KLP_Child_View(request, child_id):
	...
	kwrg = {'is_entry':True}
        #before Child.objects.all()
	resp=KLP_Child(queryset = Child.objects.filter(pk=child_id), permitted_methods = ('GET', 'POST'), responder = TemplateResponder(template_dir = 'viewtemplates', template_object_name = 'child',),)(request, child_id, **kwrg)
...
def KLP_Child_Update(request, child_id):
	...
	referKey = request.POST.get('form-0-boundary')
        #before Child.objects.all()
	KLP_Edit_Child =KLP_Child(queryset = Child.objects.filter(pk=child_id), permitted_methods = ('GET', 'POST'), responder = TemplateResponder(template_dir = 'edittemplates', template_object_name = 'child',extra_context={'buttonType':buttonType, 'referKey':referKey} ), receiver = XMLReceiver(),)
