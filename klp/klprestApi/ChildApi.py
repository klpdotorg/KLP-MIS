...
def KLP_Child_Create(request, referKey):
    ...
    KLP_Create_Child = KLP_Child(queryset, permitted_methods =
...
def KLP_Child_View(request, child_id):
    ...
    kwrg = {'is_entry': True}
    #before Child.objects.all()
    resp = KLP_Child(queryset = Child.objects.
    filter(pk=child_id), permitted_methods = ('GET', 'POST'),
...
def KLP_Child_Update(request, child_id):
    ...
    #before Child.objects.all()
    KLP_Edit_Child = KLP_Child(queryset = Child.objects.
    filter(pk=child_id), permitted_methods = ('GET', 'POST'),
...
def getStudentSearch(request, boundary, fieldName, searchtext):
    ...
    name wise to get the child details list either api/child/name/
    <child-name> or api/xml/child/name/<child-name>

    In json format api/json/child/name/<child-name>

    date of birth wise to get the child details list either api/
    child/dob/yyyymmdd or api/child/dob/yyyymmdd

    In json format api/json/child/dob/yyyymmdd

    sex wise to get the child details list either api/child/
    sex/male or api/child/sex/female

    In json format api/json/child/sex/male


    mother language wise to get the child details list either api
    /child/ml/1 or api/child/ml/2

    In json format api/json/ml/1'''
    queryset = []
    if fieldName == 'firstname':
        child_list = Child.objects.filter(firstName__startswith=searchtext,
        boundary__id = boundary).values_list('id', flat=True)

    if fieldName == 'lastname':
        child_list = Child.objects.filter(lastName__startswith=searchtext,
        boundary__id = boundary).values_list('id', flat=True)
...
def ChildrenList(request, boundary_id):
    ...
    if 'count' in itemlist:
        ...
        if 'fieldName' in itemlist or 'searchtext' in itemlist:
            ...
            template_object_name = 'child',
            extra_context = {'url': url, 'schools':
            schools, 'count': count, 'boundary':
            boundary, 'studentGroups': studentGroups}),
            entry_class = ChoiceEntry,)
            return HttpResponse(val(request))


class StdGrpFilter(Resource):
    """ To get  assessment under programme filter/
    (?P<programme_id>\d+)/programme/"""
    ...
    if studentgroup == 'None':
        ...
        if children_id and studentgroup:
            ...
            for child_id in children_id:
                ...
                studentparam = {'id': None, 'child': child,
                'school': school, 'active': 2}
...
urlpatterns = patterns('',
    ...
    KLP_Child_Create),
    url(r'^child/(?P<child_id>\d+)/view/?$',
    KLP_Child_View),
    url(r'^child/(?P<child_id>\d+)/update/?$',
    KLP_Child_Update),
    url(r'^boundary/(?P<boundary_id>\d+)/child/view/$',
    ChildrenList),
