...
asstable={2:Assessment_StudentGroup_Association,3:Assessment_Class_Association,1:Assessment_Institution_Association}

def hasChild(query, typ, boundaryType, filterBy, secFilter, permFilter, assessmentPerm, shPerm, userSel,request,showflag):
	...
	for i in query:
		if typ == 'source' or typ == 'boundary': 
		    if permFilter:
		    	...
		    	templist = [i.getPermissionChild(boundaryType), i.getAssessmentPermissionViewUrl(secFilter[0])]
		    elif shPerm:
		...
		elif  typ == 'institution' and filterBy not in [None,' ', 'None'] and permFilter in ['', ' ', None]:
                      '''
                      templist=[] 
                      for secFil in secFilter:        
			templist .append([i.getChild(),i.getStudentProgrammeUrl(filterBy, secFil)] )
                      '''
                      templist=[]
                      for secFil in secFilter:
                       ...
                       if (logUser.is_superuser or logUser.is_staff or 'AdminGroup' in user_GroupsList) :
                         ...
                         insObj=StudentGroup.objects.filter(id=i.id, active=2)[0].institution
                         permObj=   UserAssessmentPermissions.objects.filter(user__id=logUser.id,instituion=insObj, assessment=Assobj).defer('user','assessment','instituion')
                       ...
                       if assemenperm:
                         ...
                         mapObj=''
                         if Asstype==3:
                            mapObj = Assessment_StudentGroup_Association.objects.filter(assessment__id=secFil,student_group=i,active=2).defer('assessment','student_group')
                         elif Asstype==2:
                               mapObj = Assessment_Class_Association.objects.filter(assessment__id=secFil,student_group=i,active=2).defer('assessment','student_group')
                         else:
                                Instlist1=StudentGroup.objects.filter(id=i.id, active=2).defer('institution')
                                Instlist=Instlist1.values_list('institution_id', flat=True).distinct()
                                mapObj = Assessment_Institution_Association.objects.filter(assessment__id=secFil,institution__in=Instlist,active=2).defer('institution','assessment')
                                hasC=False
                                #i=Institution.objects.filter(id__in=Instlist)[0]
                         
                         if mapObj:
                            templist .append([i.getChild(),i.getStudentProgrammeUrl(filterBy, secFil)] )
       
		else:
                    '''if i.getModuleName()=='programme':
                          child=False
                          if len(GetAssementList(i.id,showflag)):
                                child=True
                     
                          templist=[child,i.getViewUrl()]
                    else:'''
                    print i.getChild()
...
def KLP_assignedAssessmentInst(userId, assessmentId):
	...
	inst_list = UserAssessmentPermissions.objects.filter(user__id=userId, assessment__id__in=assessmentId, access=True).defer('user','assessment','instituion').values_list("instituion__id", flat=True).distinct()
	return inst_list	
...
def TreeClass(request):
     ...
     klp_UserGroups = logUser.groups.all()
     user_GroupsList = ['%s' %(usergroup.name) for usergroup in klp_UserGroups]
     showflag=(logUser.is_superuser or logUser.is_staff or 'AdminGroup' in user_GroupsList) and filterBy != 'None'
	...
	if data:
	    ...
	    if (logUser.is_superuser or logUser.is_staff or 'AdminGroup' in user_GroupsList) and filterBy == 'None':
	    	...
	    	query = Boundary.objects.filter(parent__id=1,active=2, boundary_type=boundaryType).defer('boundary').order_by("name").extra(select={'lower_name':'lower(name)'}).order_by("lower_name")
	    else:
	    	if (logUser.is_superuser or logUser.is_staff or 'AdminGroup' in user_GroupsList) :
	        ...
	        try:
	        	boundary_list = Boundary.objects.filter(institution__pk__in=institutions_list, active=2, boundary_type=boundaryType).defer('boundary').values_list('parent__parent__id', flat=True).distinct()
	        except:
	        	boundary_list = Boundary.objects.filter(institution__pk__in=institutions_list, active=2, boundary_type=boundaryType).defer('boundary').values_list('parent__id', flat=True).distinct()
	        	
	        query = Boundary.objects.filter(pk__in=boundary_list, active=2, parent__id=1).defer('boundary').distinct().extra(select={'lower_name':'lower(name)'}).order_by("lower_name")
		
	else:
		# if data is not true query for all active programmes.
		query = Programme.objects.filter(active=2, programme_institution_category=boundaryType).extra(select={'lower_name':'lower(name)'}).order_by("-start_date", "-end_date", "lower_name")
		typ = 'programme'
     else:
     	# typ is not source
	if typ == 'boundary':
		...
		if (logUser.is_superuser or logUser.is_staff or 'AdminGroup' in user_GroupsList) and filterBy == 'None':
			# if logged in user is super user or staff or in AdminGroup and filterBy is none query all active boundary's where parent is 1 and based on boundaryType
			query = Boundary.objects.filter(parent__id=model[1], active=2, boundary_type=boundaryType).defer('boundary').extra(select={'lower_name':'lower(name)'}).order_by("lower_name")
		else:
			if (logUser.is_superuser or logUser.is_staff or 'AdminGroup' in user_GroupsList):
				# if logged in user is super user or staff or in AdminGroup and filterBy is not none query all active SG's based on assessments
				#studentgroup_list = Assessment_StudentGroup_Association.objects.filter(assessment__id__in=secFilter, active=2).values_list('student_group', flat=True).distinct()
			...
			if parentBoundary.boundary_category.boundary_category in ['district',]:
				...
				boundary_list = Boundary.objects.filter(institution__pk__in=institutions_list, active=2, boundary_type=boundaryType).defer('boundary').values_list('parent', flat=True).distinct()
				query = Boundary.objects.filter(parent__id=model[1], pk__in = boundary_list, active=2, boundary_type=boundaryType).defer('boundary').distinct().extra(select={'lower_name':'lower(name)'}).order_by("lower_name")
			else:
				boundaries = Boundary.objects.filter(parent__id=model[1], institution__pk__in=institutions_list, active=2, boundary_type=boundaryType).defer('boundary').values_list('id', flat=True).distinct()
				query = Boundary.objects.filter(id__in=boundaries).defer('boundary').extra(select={'lower_name':'lower(name)'}).order_by("lower_name")	
		...
		# if typ is programme Query For active assessment based On programme id
                ...
                if showflag:
                               activelist=[1,2]
		query = Assessment.objects.filter(programme__id=model[1],active__in= activelist)
	elif typ == 'assessment':
     ...
     CDict=hasChild(query, typ, boundaryType, filterBy, secFilter, permFilter, assessmentPerm, shPerm, userSel,request,showflag)  # Checking for child objects
     val= Collection(
     queryset = query,
     responder = TreeResponder(CDict=CDict),
     )
     return HttpResponse(val(request),mimetype="application/json")
asstable={3:Assessment_StudentGroup_Association,2:Assessment_Class_Association,1:Assessment_Institution_Association}
selectfield={2:'student_group_id',3:'student_group__id',1:'institution__id'}
def getAssInst(secFilter,superuser=1):
 	...
 	institutions_listall=[]
  	for k in secFilter:
   		...
   		asstype=assobj.typ
                print asstype 
    		studentgroup_list = asstable[asstype].objects.filter(assessment=assobj, active=2).values_list(selectfield[asstype], flat=True).distinct()
    		# Query institutions based SG's
    		if asstype in [2,3]:
      			institutions_list = StudentGroup.objects.filter(id__in=studentgroup_list, active=2).values_list('institution__id', flat=True).distinct()
    		else:
    		...
    		institutions_listall= list(set(institutions_listall)|set(institutions_list))
  	return institutions_listall  
def getAssSG(secFilter,instid):
        flag=0
        sg_listall=[]
        sg=StudentGroup.objects.filter(institution__id=instid,active=2).values_list('id', flat=True).distinct()
        for k in secFilter:
                assobj=Assessment.objects.get(id=k)
                asstype=assobj.typ
                # Query institutions based SG's
                if asstype in [2,3]:
                        studentgroup_list = asstable[asstype].objects.filter(assessment=assobj,student_group__id__in=sg).values_list('student_group__id', flat=True).distinct()
                        sg_list = StudentGroup.objects.filter(id__in=studentgroup_list, active=2).defer("institution").values_list('id', flat=True).distinct()
                else:
                        if sg:
                                                             sg_list=[sg[0]]
...
def GetAssementList(programId,showflag=False):
    ...
    if showflag:
                              activelist=[1,2]
    return Assessment.objects.filter(programme__id=programId,active__in=activelist).values_list('id',flat=True)
