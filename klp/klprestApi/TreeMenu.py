        ...
        '', ' ', None]:
            ...
            for secFil in secFilter:
                ...
                if mapObj:
                    ...
                    i.getStudentProgrammeUrl(filterBy, secFil)])
        else:
...
def KLP_assignedInstitutions(userId):
    ...
    rawQuerySet = Institution.objects.raw
    (""" SELECT "id","obj_id" FROM "public".
    "object_permissions_institution_perms" WHERE "user_id"
    = '%s' AND "Acess" = 't' """ % (userId))
    inst_list = []
...
def TreeClass(request):
    ...
    if filterBy != 'None' and secFilter != 'None':
        ...
        if not query:
            ...
            if (logUser.is_superuser or logUser.is_staff or
            'AdminGroup' in user_GroupsList) and filterBy == 'None':
                # if logged in user is super user or staff
                #or in AdminGroup and filterBy is none query
                #all active institutions's based on boundary
                query = Institution.objects.filter(boundary__id=
                model[1], active=2).extra(select={'lower_name':
            ...
            # if typ is assessment Query For active Questions
