...
def KLP_Assessment_View(request, assessment_id):
    ...
    resp = KLP_Assessment(queryset = Assessment.objects.
...
def KLP_Assessment_Create(request, referKey):
    """ To Create New Assessment programme/assessment/(?P<referKey>\d+)
    /creator/"""
    ...
    if currentMont>4:
        ...
        print referKey
        #before Assessment.objects.all()
        KLP_Create_Assessment = KLP_Assessment(queryset = Assessment.
        objects.filter(pk=0), permitted_methods = ('GET', 'POST'),
        responder = TemplateResponder(template_dir = 'viewtemplates',
...
def KLP_Assessment_Update(request, assessment_id):
    """ To update Selected Boundary assessment/(?P<assessment_id>
    \d+)/update/"""
    ...
    if currentMont>4:
        ...
        Assessment.objects.filter(pk=assessment_id), permitted_methods =
        ('GET', 'POST'), responder = TemplateResponder(template_dir =
        'edittemplates', template_object_name = 'assessment',
        extra_context={'buttonType': buttonType, 'referKey': referKey,
        'endDate': 30, 'endYear': endYear, 'endMonth': 'APRIL'}),
        receiver = XMLReceiver(),)
        response = KLP_Edit_Assessment.responder.update_form(request,
        pk=assessment_id, form_class=Assessment_Form)

    return HttpResponse(response)


class KLP_Get_Assessments(Resource):
    """ To get  assessment under programme filter/programme/
    (?P<programme_id>\d+)/assessments/"""

    def read(self, request, programme_id):
        try:
            # Query all active(2) assessments based on programme id
            assessments_list = Assessment.objects.filter(programme__id
