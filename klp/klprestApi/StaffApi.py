...
def KLP_Staff_View(request, staff_id):
    ...
    resp = KLP_Boundary(queryset=Staff.objects.
    filter(pk=staff_id), permitted_methods=('GET', 'POST'),
    responder = TemplateResponder(template_dir='viewtemplates',
    template_object_name = 'staff',))(request, staff_id, **kwrg)
...
def KLP_staff_list(request, institution_id):
    """ To view list of staff in school school
    /(?P<school_id>\d+)/staff/view/"""
    queryset = Staff.objects.filter(institution__id =
    institution_id, active=2).order_by('firstName')
...
def KLP_Staff_Update(request, staff_id):
    ...
    boundary_category.lower() == 'circle':
        ...
        permitted_methods = ('GET', 'POST'), responder =
