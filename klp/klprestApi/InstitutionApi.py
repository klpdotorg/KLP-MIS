...
def KLP_Institution_View(request, institution_id):
	...
	kwrg = {'is_entry':True}
        #before Institution.objects.all()
	resp=KLP_Institution(queryset = Institution.objects.filter(pk=institution_id), permitted_methods = ('GET', 'POST'), responder = TemplateResponder(template_dir = 'viewtemplates', template_object_name = 'institution',),)(request, institution_id, **kwrg)
