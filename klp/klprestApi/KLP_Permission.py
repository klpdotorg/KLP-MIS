...
def KLP_Assign_Permissions(request):
	...
	# get permission type
        ...
        message="A mail will be sent to %s as soon as all the permissions are assigned" % (receiver)
	...
	assessmentPerm = request.POST.get('assessmentPerm')
        print deUserList,permissions,message,permissionType,assessmentId,bound_cat,inst_list,bound_list,assessmentPerm  
...
def KLP_Users_list(request):
	...
	if user.id:
		...
		user_list = User.objects.filter( is_staff=0, is_superuser=0).order_by("username")
...
def KLP_User_Activate(request, user_id):
        ...
        if user.id:
                ...
                return render_to_response('viewtemplates/userAction_done.html',{'user':request.user,'selUser':userObj,'message':'User Activated Successfully', 'legend':'Karnataka Learning Partnership', 'entry':"Add"}, context_instance=RequestContext(request))
