...
class SerializeResponder(object):
    ...
    """
    def __init__(self, format, mimetype=None, paginate_by=None,
    allow_empty=False):
        """
        format:
...
class TemplateResponder(object):
    ...
    def element(self, request, elem):
        ...
        populate_xheaders(request, response, elem.__class__, getattr(
        elem, elem._meta.pk.name))
        return response
    ...
    if request.POST:
        ...
        #form = ResourceForm(request.POST, queryset=
        ...
        # Check if current model is child then pass ChildTrue
        #to True else false
        ChildTrue = modelName == 'child' and True or False
        if  ChildTrue:
            # Check for realtion names if names are not there validation fail
            if not (request.POST.get('form-0-fatherfirstname', '') or
            request.POST.get('form-0-motherfirstname', '')):
                RelationValid = False
                IValid = False
                print ChildTrue
        if modelName == 'institution':
            # If Institution Check for address information and if
            #it there update data
            if request.POST.get('form-0-address') and request.POST.get(
            'form-0-languages') and request.POST.get('form-0-name'):
                ...
                if addressObj:
                    ...
                    addressObj.instidentification = request.POST.get(
                    'form-0-instidentification')
                    addressObj.landmark = request.POST.get('form-0-landmark')
                    addressObj.routeInformation = request.POST.get(
                    ...
                    'form-0-routeInformation'))
                    addressObj.save()
                    new_data['form-0-inst_address'] = addressObj.id_Student_
                IValid = True
                #Valid = True
                # If request method is post, post data for object
                form = ResourceForm(
                request.POST, queryset=queryset.model.objects.filter(pk=pk), )
                Valid = form.is_valid()  # Validate form
        ...
        if Valid and IValid and RelationValid:
            ...
            """if modelName=='institution':
            # if model isinstitution  pass address id to the institution
            to save data
            #new_data = request.POST.copy()
            #new_data['form-0-inst_address'] = addressObj.id
            #form = ResourceForm(new_data, queryset=queryset.model.objects.
            filter(pk=pk),)
            obj = form.save()[0]
            else:
            # else save data
            ...
            if modelName == 'child':
                ...
                for rel_type, rel_value in relation.iteritems():
                    ...
                    if request.POST[rel_type]:
                        ...
                        relation_type=rel_value, child=obj)

                        if relation:

                            rel = relation.update(first_name=request.POST[
                            rel_type], middle_name=request.POST[names[
                            rel_value + '-MiddleName']], last_name=
                            request.POST[
                ...
                for clas in newclasses:
                    if clas not in mapClasIds:
                        # if newly assigned class not in already assigned
                        #class create relation ship with SG.
                        clasObj = StudentGroup.objects.get(pk=clas)
                        staff_StudentGroup = Staff_StudentGroupRelation(
                        staff = elem, student_group = clasObj,
            ...
            print buttonType
            if buttonType == 'save':
            #If Button type is Save show detail information
            #about updated object_list
...
# Show the form
        ...
        return render_to_response(template_name,
                ...
                self.template_object_name: elem, 'extra_context':
                self.extra_context}, context_instance=RequestContext(request))
