...
def KLP_Question_View(request, question_id):
	...
	resp=KLP_Question(queryset = Question.objects.filter(pk=question_id), permitted_methods = ('GET', 'POST'), responder = TemplateResponder(template_dir = 'viewtemplates', template_object_name = 'question',),)(request, question_id, **kwrg)
...
def KLP_Question_Create(request, referKey):
	...
	order = Question.objects.filter(assessment__id=referKey).count()+1
        #before Question.objects.all()
        KLP_Create_Question = KLP_Question(queryset = Question.objects.filter(pk=0), permitted_methods = ('GET', 'POST'), responder = TemplateResponder(template_dir = 'viewtemplates', template_object_name = 'question', extra_context={'buttonType':buttonType, 'referKey':referKey, 'order':order}), receiver = XMLReceiver(),)
...
def KLP_Question_Update(request, question_id):
	...
	referKey = request.POST.get('form-0-assessment')
        #before Question.objects.all()
	KLP_Edit_Question =KLP_Question(queryset = Question.objects.filter(pk=question_id), permitted_methods = ('GET', 'POST'), responder = TemplateResponder(template_dir = 'edittemplates', template_object_name = 'question', extra_context={'buttonType':buttonType, 'referKey':referKey}), receiver = XMLReceiver(),)
