...
def KLP_Question_View(request, question_id):
    ...
    resp = KLP_Question(queryset=Question.objects.
    filter(pk=question_id), permitted_methods
    = ('GET', 'POST'), responder=
    TemplateResponder(template_dir= 'viewtemplates',
    template_object_name= 'question',),)(request, question_id, **kwrg)
    return HttpResponse(resp)


def KLP_Question_Create(request, referKey):
    """ To Create New Question assessment/question/
    ...
    #before Question.objects.all()
    KLP_Create_Question = KLP_Question(queryset =
    Question.objects.filter(pk=0), permitted_methods
    = ('GET', 'POST'), responder =
    TemplateResponder(template_dir = 'viewtemplates',
...
def KLP_Question_Update(request, question_id):
    ...
    #before Question.objects.all()
    KLP_Edit_Question = KLP_Question(queryset =
    Question.objects.filter(pk=question_id), permitted_methods
    = ('GET', 'POST'), responder =
    TemplateResponder(template_dir = 'edittemplates',
    template_object_name = 'question', extra_context
