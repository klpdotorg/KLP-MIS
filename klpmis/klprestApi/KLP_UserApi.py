...
post_change_redirect=None):
    ...
    user_GroupsList = ['%s' % (usergroup.name) for usergroup
    in klp_UserGroups]
    if user.id is not None and (user.is_superuser or
    'AdminGroup' in user_GroupsList):
        ...
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                {'form': form, 'title': 'KLP User', 'legend':
                'Karnataka Learning Partnership', 'entry': "Add"},
                context_instance=RequestContext(request))
        else:
            form = UserCreationFormExtended()
            return render_to_response(template_name,
            {'form': form, 'title': 'KLP User', 'legend':
            'Karnataka Learning Partnership', 'entry':
            "Add"}, context_instance=RequestContext(request))
    else:
        # if use is not login and user is not super user
        #or not in admin group
...
def KLP_addNewUser_done(request):
    ...
    'title': 'KLP User', 'legend':
    'Karnataka Learning Partnership', 'entry': "Add"},
    context_instance=RequestContext(request))


def KLP_password_change(request, template_name=
    ...
    if user.id is not None:
        ...
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                return render_to_response(template_name,
                {'form': form, 'returnUrl': returnUrl,
                'title': 'KLP Change Password', 'legend':
                'Karnataka Learning Partnership', 'entry': "Add"},
                context_instance=RequestContext(request))
        else:
...
def KLP_password_change_done(request, template_name=
'viewtemplates/password_change_done.html'):
    ...
    if user.is_superuser:
        ...
        returnUrl = usrUrl[userGroup]
    return render_to_response(template_name,
    {'returnUrl': returnUrl, 'title': 'KLP Change Password',
