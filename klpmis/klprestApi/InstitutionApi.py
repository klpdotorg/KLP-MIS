...
def KLP_Institution_Create(request, referKey):
    """ To Create New Institution boundary/(?P<referKey>\d+)
    /institution/creator/"""
    ...
    if boundaryObj.boundary_category.boundary_category.lower() == 'circle':
        # if boundary category is circle then institutionType
        #is Anganwadi and  categoryType is 2 else institutionType is
        #Institution and  categoryType is 1
        ...
        template_object_name = 'institution', extra_context=
...
def KLP_Institution_View(request, institution_id):
    """ To View Selected Institution institution/
    (?P<institution_id>\d+)/view/?$"""
    kwrg = {'is_entry': True}
    #before Institution.objects.all()
    resp = KLP_Institution(queryset = Institution.objects.
    filter(pk=institution_id), permitted_methods = ('GET', 'POST'),
    responder = TemplateResponder(template_dir = 'viewtemplates',
    template_object_name = 'institution',),)(request, institution_id, **kwrg)
    return HttpResponse(resp)


def KLP_Institution_Update(request, institution_id):
    """ To update Selected Institution institution/
    (?P<institution_id>\d+)/update/"""
    # Checking user Permissions
    ...
    if institutionObj.boundary.boundary_category.boundary_category == 'Circle':
        ...
        categoryType)
        #before Institution.objects.all()
        KLP_Edit_Institution = KLP_Institution(queryset =
        Institution.objects.filter(pk=institution_id), permitted_methods
        = ('GET', 'POST'), responder = TemplateResponder(template_dir
        = 'edittemplates', template_object_name = 'institution',
        extra_context = {'buttonType': buttonType,
        'referKey': referKey, 'institutionType': institutionType,
        'categoryList': categoryList, 'selCategoryTyp': selCategoryTyp}),
...
def KLP_Institution_Boundary(request, boundary_id, permissionType,
assessment_id=None):
    ...
    if user.is_superuser or 'AdminGroup' in klp_GroupsList:
        ...
        users = User.objects.filter(groups__name__in
        =['Data Entry Executive', 'Data Entry Operator'],
        is_active=1).order_by("username")
        # get Selected boundary object
        boundaryObj = Boundary.objects.get(id=boundary_id)
        respDict = {'users': users, 'boundary': boundaryObj,
        'permissionType': permissionType, 'url': request.path}
