...
def KLP_Student_Create(request, studentgroup_id, counter=0):
    """ To Create New Student boundary/(?P<bounday>\d+)
...
def KLP_Student_View(request, student_id):
    """ To View Selected Student studentsroup/
    (?P<studentsroup_id>\d+)/view/?$"""
    kwrg = {'is_entry': True}
    #before Student.objects.all()
    resp = KLP_Student(queryset =
    ...
    if len(students_list) > 0:
        for stud_id in students_list:
            ...
            try:
                ...
                count = count + 1
                delFailed += '%s %s ,'(obj.child.firstName,
                obj.child.lastName)
    if count == 0:
        return HttpResponse
