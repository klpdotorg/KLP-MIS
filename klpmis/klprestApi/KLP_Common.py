...
2) To Delete (deactive object) boundary, institution, sg,
staff, programme, assessment and question.
"""
...
class KLP_Create_Node(Resource):
    ...
    def read(self, request, model_name, new_id):
        """ This method uses to create new node for tree on
        creation of  boundary,  institution, programme, assessment,
        question and  studentgroup"""
        objDict = {'boundary': Boundary, 'institution': Institution,
        'programme': Programme, 'assessment': Assessment,
        'question': Question, 'studentgroup': StudentGroup, }
...
class KLP_Delete(Resource):
    ...
    def read(self, request, model_name1, referKey):
        modelDict = {'boundary': Boundary, 'institution': Institution,
        'programme': Programme, 'assessment': Assessment,
        'question': Question, 'studentgroup': StudentGroup,
        'student': Student, 'staff': Staff,
        'class': StudentGroup, 'center': StudentGroup}
        # Checking user Permissions
        KLP_user_Perm(request.user, modelDict[model_name1.
        lower()]._meta.module_name, "Delete")
        ...
        if model_name1.lower() == 'boundary':
            ...
            (student__id = referKey).update(active=0)
            if model_name1.lower() in [ 'class','center']:
            Staff_StudentGroupRelation.objects.
