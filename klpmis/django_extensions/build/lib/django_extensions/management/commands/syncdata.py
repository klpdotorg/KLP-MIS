...
class Command(BaseCommand):
    ...
    help = \
        'Makes the current database have the same data as the fixture(s), no more, no less.'
    ...
    def handle(self, *fixture_labels, **options):
        ...
        humanize = lambda dirname: dirname and "'%s'" % dirname \
            or 'absolute path'
        ...
        for fixture_label in fixture_labels:
            ...
            if len(parts) == 1:
                ...
                (fixture_name, format) = ('.'.join(parts[:-1]),
                        parts[-1])
                if format \
                    in serializers.get_public_serializer_formats():
                    formats = [format]
                else:
                    formats = []
            ...
            if formats:
                ...
                sys.stderr.write(self.style.ERROR("Problem installing fixture '%s': %s is not a known "
                ...
                for format in formats:
                    ...
                    if verbosity > 1:
                        print "Trying %s for %s fixture '%s'..." \
                            % (humanize(fixture_dir), format,
                               fixture_name)
                    try:
                        full_path = os.path.join(fixture_dir,
                        ...
                        if label_found:
                            ...
                            try:
                                ...
                                self.remove_objects_not_in(objects_to_keep,
                                        verbosity)
        ...
        if 0 in objects_per_fixture:
            sys.stderr.write(self.style.ERROR("No fixture data found for '%s'. (File format may be invalid.)"
                              % fixture_name))
            transaction.rollback()
            transaction.leave_transaction_management()
            return
        ...
        if object_count > 0:
            sequence_sql = \
                connection.ops.sequence_reset_sql(self.style, models)
            if sequence_sql:
                if verbosity > 1:
...
if not [opt for opt in Command.option_list if opt.dest == 'verbosity']:
    Command.option_list += (make_option(
        '--verbosity',
        '-v',
        action='store',
        dest='verbosity',
