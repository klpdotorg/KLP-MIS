...
class Command(NoArgsCommand):
    ...
    option_list = NoArgsCommand.option_list + (
        make_option('--email', dest='default_email',
        ...
        make_option('--include-groups', dest='include_groups',
                    default=None,
                    help='Include users matching this group. (use comma seperation for multiple groups)'
                    ),
        make_option('--exclude-groups', dest='exclude_groups',
                    default=None,
    ...
    def handle_noargs(self, **options):
        ...
        if exclude_groups:
            groups = \
                Group.objects.filter(name__in=exclude_groups.split(','))
            if groups:
        ...
        if include_groups:
            groups = \
                Group.objects.filter(name__in=include_groups.split(','))
            if groups:
