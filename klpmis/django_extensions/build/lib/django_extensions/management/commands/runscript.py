...
class Command(BaseCommand):
    ...
    option_list = BaseCommand.option_list + (make_option('--fixtures',
            action='store_true', dest='infixtures', default=False,
            help='Only look in app.fixtures subdir'),
            make_option('--noscripts', action='store_true',
            dest='noscripts', default=False,
            help='Look in app.scripts subdir'), make_option(
        '-s',
        '--silent',
        action='store_true',
        dest='silent',
        ...
        def run_script(mod):
            ...
            try:
                ...
                if silent:
                    return
                if verbosity > 0:
                    print ERROR("Exception while running run() in '%s'"
                                % mod.__name__)
                if show_traceback:
                    raise
        ...
        def my_import(mod):
            ...
            try:
                ...
                if hasattr(t, 'run'):
                    ...
                    if verbosity > 1:
                        print ERROR2("Find script '%s' but no run() function found."
                                 % mod)
            except ImportError:
                return False
        ...
        def find_modules_for_script(script):
            ...
            for app in get_apps():
                ...
                for subdir in subdirs:
                    mod = my_import('.'.join(app_name + [subdir,
                                    script]))
                    if mod:
                        modules.append(mod)
        ...
        for script in scripts:
            ...
            if not modules:
                if verbosity > 0 and not silent:
                    print ERROR("No module for script '%s' found"
                                % script)
            for mod in modules:
                if verbosity > 1:
                    print NOTICE2("Running script '%s' ..."
                                  % mod.__name__)
                run_script(mod)
