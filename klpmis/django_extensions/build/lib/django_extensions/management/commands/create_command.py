...
class Command(AppCommand):
    ...
    help = \
        'Creates a Django management command directory structure for the given app name in the current directory.'
    args = '[appname]'
    ...
    def handle_app(self, app, **options):
        ...
        copy_template('command_template', project_dir,
                      options.get('command_name'), '%sCommand'
...
def copy_template(
    ...
    for (d, subdirs, files) in os.walk(template_dir):
        ...
        for f in files:
            ...
            fp_new.write(fp_old.read().replace('{{ command_name }}',
                         command_name).replace('{{ base_command }}',
                         base_command).replace('{{ handle_method }}',
                         handle_method))
            fp_old.close()
            fp_new.close()
            try:
