...
def import_job(app_name, name, when=None):
    jobmodule = '%s.jobs.%s%s' % (app_name, when and '%s.' % when or ''
                                  , name)
    ...
    try:
        ...
        raise JobError("Job module %s does not contain class instance named 'Job'"
                        % jobmodule)
                    ...
                    try:
                        ...
                        for name in find_jobs(path):
                            if (app_name, name) in _jobs:
                                raise JobError('Duplicate job %s'
                                        % name)
...
def print_jobs(
    ...
    when_spacer = '%%-%is' % max(len(e.when) for e in jobmap.values()
                                 if e.when)
    if show_header:
        line = ' '
        if show_appname:
