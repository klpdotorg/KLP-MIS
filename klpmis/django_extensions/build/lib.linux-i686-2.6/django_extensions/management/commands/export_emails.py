...
FORMATS = ['address', 'google', 'outlook', 'linkedin', 'vcard']
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if group and not Group.objects.filter(name=group).count() == 1:
            names = u"', '".join(g['name'] for g in
                                 Group.objects.values('name'
                                 )).encode('utf-8')
            if names:
                names = "'" + names + "'."
            raise CommandError("Unknown group '" + group
                               + "'. Valid group names are: " + names)
    ...
    def vcard(self, qs, out):
        ...
        for ent in qs:
            ...
            if not ent['last_name'] and not ent['first_name']:
                ...
                card.add('n').value = \
                    vobject.vcard.Name(full_name(**ent))
            else:
