...
class Command(LabelCommand):
    ...
    option_list = LabelCommand.option_list + (make_option('--list', '-l'
            , action='store_true', dest='list_jobs',
    ...
    def runjob(
        ...
        try:
            ...
            import traceback
            print 'ERROR OCCURED IN JOB: %s (APP: %s)' % (job_name,
                    app_name)
    ...
    def handle(self, *args, **options):
        ...
        if options.get('list_jobs'):
            print_jobs(only_scheduled=False, show_when=True,
                       show_appname=True)
        else:
            if not job_name:
