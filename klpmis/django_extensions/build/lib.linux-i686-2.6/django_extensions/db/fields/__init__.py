...
class AutoSlugField(SlugField):
    ...
    def create_slug(self, model_instance, add):
        ...
        if add or self.overwrite:
            ...
            slug_for_field = lambda field: \
                self.slugify_func(getattr(model_instance, field))
            slug = self.separator.join(map(slug_for_field,
                    self._populate_from))
            next = 2
        else:
            ...
            slug = self._slug_strip(getattr(model_instance,
                                    self.attname))
            next = slug.split(self.separator)[-1]
            if next.isdigit():
                slug = \
                    self.separator.join(slug.split(self.separator)[:-1])
                next = int(next)
            else:
                next = 2
