...
class SQLDiff(object):
    ...
    SQL_FIELD_MISSING_IN_DB = lambda self, style, qn, args: \
        '%s %s\n\t%s %s %s;' % (style.SQL_KEYWORD('ALTER TABLE'),
    ...
    SQL_FIELD_MISSING_IN_MODEL = lambda self, style, qn, args: \
        '%s %s\n\t%s %s;' % (style.SQL_KEYWORD('ALTER TABLE'),
                             style.SQL_TABLE(qn(args[0])),
                             style.SQL_KEYWORD('DROP COLUMN'),
                             style.SQL_FIELD(qn(args[1])))
    SQL_INDEX_MISSING_IN_DB = lambda self, style, qn, args: \
        '%s %s\n\t%s %s (%s);' % (style.SQL_KEYWORD('CREATE INDEX'),
                                  style.SQL_TABLE(qn('%s_idx'
                                  % '_'.join(args[0:2]))),
    ...
    SQL_INDEX_MISSING_IN_MODEL = lambda self, style, qn, args: '%s %s;' \
        % (style.SQL_KEYWORD('DROP INDEX'), style.SQL_TABLE(qn('%s_idx'
           % '_'.join(args[0:2]))))
    ...
    SQL_FIELD_PARAMETER_DIFFER = lambda self, style, qn, args: \
        '%s %s\n\t%s %s %s;' % (style.SQL_KEYWORD('ALTER TABLE'),
                                ...
                                style.SQL_FIELD(qn(args[1])),
                                style.SQL_COLTYPE(args[2]))
    SQL_COMMENT = lambda self, style, qn, args: \
        style.NOTICE('-- Comment: %s' % style.SQL_TABLE(args[0]))
    SQL_TABLE_MISSING_IN_DB = lambda self, style, qn, args: \
        style.NOTICE('-- Table missing: %s' % args[0])
    ...
    def __init__(self, app_models, options):
        ...
        self.django_tables = \
            self.get_django_tables(options.get('only_existing', True))
    ...
    def get_django_tables(self, only_existing):
        try:
            django_tables = \
                self.introspection.django_table_names(only_existing=only_existing)
        except AttributeError:
            ...
            try:
                django_tables = \
                    _sql.django_table_names(only_existing=only_existing)
            except AttributeError:

                # backwards compatibility for before svn r7568

                django_tables = \
                    _sql.django_table_list(only_existing=only_existing)
    ...
    def get_field_db_type(
        ...
        if type_code in self.DATA_TYPES_REVERSE_OVERRIDE:
            ...
            try:
                try:
                    reverse_type = \
                        self.introspection.data_types_reverse[type_code]
                except AttributeError:

                    # backwards compatibility for before introspection refactoring (r8296)

                    reverse_type = \
                        self.introspection.DATA_TYPES_REVERSE.get(type_code)
            except KeyError:
        ...
        field_db_type = getattr(models,
                                reverse_type)(**kwargs).db_type()
    ...
    def find_unique_missing_in_db(
        ...
        for field in meta.fields:
            if field.unique:
                ...
                if attname in table_indexes \
                    and table_indexes[attname]['unique']:
                    continue
                self.add_difference('unique-missing-in-db', table_name,
                                    attname)
    ...
    def find_unique_missing_in_model(
        ...
        for (att_name, att_opts) in table_indexes.iteritems():
            if att_opts['unique'] and att_name in fields \
                and not fields[att_name]:
                if att_name in flatten(meta.unique_together):
                    continue
                self.add_difference('unique-missing-in-model',
                                    table_name, att_name)
        ...
        for field in meta.fields:
            if field.db_index:
                ...
                if not attname in table_indexes:
                    self.add_difference('index-missing-in-db',
                            table_name, attname)
        ...
        for (att_name, att_opts) in table_indexes.iteritems():
            if att_name in fields:
                ...
                if att_opts['unique'] and field.unique:
                    continue
                if att_opts['unique'] and att_name \
                    in flatten(meta.unique_together):
                    continue
                self.add_difference('index-missing-in-model',
                                    table_name, att_name)
    ...
    def find_field_missing_in_model(
        ...
        for row in table_description:
            if row[0] not in fieldmap:
                self.add_difference('field-missing-in-model',
                                    table_name, row[0])
    ...
    def find_field_missing_in_db(
        ...
        for (field_name, field) in fieldmap.iteritems():
            if field_name not in db_fields:
                self.add_difference('field-missing-in-db', table_name,
                                    field_name, field.db_type())
    ...
    def find_field_type_differ(
        ...
        for field in meta.fields:
            ...
            model_type = \
                self.strip_parameters(self.get_field_model_type(field))
            db_type = \
                self.strip_parameters(self.get_field_db_type(description,
                    field))

            # use callback function if defined

            if func:
                (model_type, db_type) = func(field, description,
                        model_type, db_type)

            if not model_type == db_type:
                self.add_difference('field-type-differ', table_name,
                                    field.name, model_type, db_type)
    ...
    def find_field_parameter_differ(
        ...
        for field in meta.fields:
            ...
            if not self.strip_parameters(model_type) \
                == self.strip_parameters(db_type):
                continue
            ...
            if not model_type == db_type:
                self.add_difference('field-parameter-differ',
                                    table_name, field.name, model_type,
                                    db_type)
    ...
    def find_differences(self):
        ...
        for app_model in self.app_models:
            ...
            table_indexes = self.introspection.get_indexes(self.cursor,
                    table_name)
            fieldmap = dict([(field.db_column or field.get_attname(),
            ...
            try:
                table_description = \
                    self.introspection.get_table_description(self.cursor,
                        table_name)
            except Exception, e:
                model_diffs.append((app_model.__name__,
                                   [str(e).strip()]))
            ...
            self.find_unique_missing_in_model(meta, table_indexes,
                    table_name)

            # 2) find: 'index-missing-in-model'

            self.find_index_missing_in_model(meta, table_indexes,
                    table_name)

            # 3) find: 'field-missing-in-model'

            self.find_field_missing_in_model(fieldmap,
                    table_description, table_name)

            # Fields which are defined in models but not in database
            # 4) find: 'field-missing-in-db'

            self.find_field_missing_in_db(fieldmap, table_description,
                    table_name)

            # 5) find: 'unique-missing-in-db'

            self.find_unique_missing_in_db(meta, table_indexes,
                    table_name)

            # 6) find: 'index-missing-in-db'

            self.find_index_missing_in_db(meta, table_indexes,
                    table_name)
            ...
            self.find_field_parameter_differ(meta, table_description,
                    table_name)
    ...
    def print_diff_text(self, style):
        ...
        for (app_label, model_name, diffs) in self.differences:
            ...
            if not self.dense and cur_app_label != app_label:
                print style.NOTICE('+ Application:'), \
                    style.SQL_TABLE(app_label)
                cur_app_label = app_label
            if not self.dense:
                print style.NOTICE('|-+ Differences for model:'), \
                    style.SQL_TABLE(model_name)
            for diff in diffs:
                (diff_type, diff_args) = diff
                text = self.DIFF_TEXTS[diff_type] % dict((str(i),
                        style.SQL_TABLE(e)) for (i, e) in
                        enumerate(diff_args))
                text = "'".join(i % 2 == 0 and style.ERROR_OUTPUT(e)
                                or e for (i, e) in
                                enumerate(text.split("'")))
                if not self.dense:
                    print style.NOTICE('|--+'), text
                else:
                    print style.NOTICE('App'), \
                        style.SQL_TABLE(app_name), style.NOTICE('Model'
    ...
    def print_diff_sql(self, style):
        ...
        for (app_label, model_name, diffs) in self.differences:
            if not diffs:
                continue
            if not self.dense and cur_app_label != app_label:
                print style.NOTICE('-- Application: %s'
                                   % style.SQL_TABLE(app_label))
                cur_app_label = app_label
            if not self.dense:
                print style.NOTICE('-- Model: %s'
                                   % style.SQL_TABLE(model_name))
            for diff in diffs:
                (diff_type, diff_args) = diff
...
class MySQLDiff(SQLDiff):
    ...
    def get_field_db_type(
        ...
        if field:
            ...
            if self.strip_parameters(field_type) == 'char' \
            ...
            if db_type == 'integer' and description[1] \
                == FIELD_TYPE.TINY and description[4] == 1:
                db_type = 'bool'
            if db_type == 'integer' and description[1] \
                == FIELD_TYPE.SHORT:
...
class SqliteSQLDiff(SQLDiff):
    ...
    def get_field_db_type(
        ...
        db_type = super(SqliteSQLDiff,
                        self).get_field_db_type(description)
        if not db_type:
            return
        if field:
            field_type = self.get_field_model_type(field)
...
class PostgresqlSQLDiff(SQLDiff):
    ...
    DATA_TYPES_REVERSE_OVERRIDE = {20: 'IntegerField',
                                   1042: 'CharField'}
    ...
    def get_field_db_type(
        ...
        db_type = super(PostgresqlSQLDiff,
                        self).get_field_db_type(description)
        if not db_type:
            return
        if field:
            ...
            if table_name:
                ...
                if tablespace == '':
                    tablespace = 'public'
                check_constraint = \
                    self.check_constraints.get((tablespace, table_name,
                        field.attname), {}).get('pg_get_constraintdef',
                        None)
                if check_constraint:
                    ...
                    check_constraint = check_constraint.replace('))',
                            ')')
                    check_constraint = '("'.join([')' in e
                            and '" '.join(e.split(' ', 1)) or e
...
class Command(BaseCommand):
    ...
    option_list = BaseCommand.option_list \
        + (make_option('--all-applications', '-a', action='store_true',
           ...
           ), make_option('--not-only-existing', '-e',
           action='store_false', dest='only_existing',
           help='Check all tables that exist in the database, not only tables that should exist based on models.'
           ), make_option('--dense-output', '-d', action='store_true',
           dest='dense_output',
    ...
    help = \
        """Prints the (approximated) difference between models and fields in the database for the given app name(s).
...
to check/debug ur models compared to the real database tables and columns."""
    ...
    def handle(self, *app_labels, **options):
        ...
        if options.get('all_applications', False):
            ...
            try:
                app_list = [models.get_app(app_label) for app_label in
                            app_labels]
            except (models.ImproperlyConfigured, ImportError), e:
                raise CommandError('%s. Are you sure your INSTALLED_APPS setting is correct?'
                                    % e)
        ...
        cls = DATABASE_SQLDIFF_CLASSES.get(settings.DATABASE_ENGINE,
                GenericSQLDiff)
