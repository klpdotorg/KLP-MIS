...
class KCacheGrind(object):
    ...
    def _subentry(self, lineno, subentry):
        ...
        if isinstance(code, str):
            ...
            print >> out_file, 'calls=%d %d' % (subentry.callcount,
                    code.co_firstlineno)
...
class Command(BaseCommand):
    ...
    option_list = BaseCommand.option_list + (
        ...
        make_option('--nomedia', action='store_true', dest='no_media',
                    default=False,
                    help='Do not profile MEDIA_URL and ADMIN_MEDIA_URL'
                    ),
        make_option('--use-cprofile', action='store_true',
                    dest='use_cprofile', default=False,
                    help='Use cProfile if available, this is disabled per default because of incompatibilities.'
                    ),
        make_option('--kcachegrind', action='store_true',
                    dest='use_lsprof', default=False,
    ...
    def handle(
        ...
        **options
        ...
        quit_command = sys.platform == 'win32' and 'CTRL-BREAK' \
            or 'CONTROL-C'
        ...
        def inner_run():
            ...
            def make_profiler_handler(inner_handler):
                ...
                def handler(environ, start_response):
                    ...
                    if no_media and (path_info.startswith('/media')
                            or path_info.startswith(settings.MEDIA_URL)):
            ...
            try:
                path = admin_media_path or django.__path__[0] \
                    + '/contrib/admin/media'
                handler = \
                    make_profiler_handler(AdminMediaHandler(WSGIHandler(),
                        path))
