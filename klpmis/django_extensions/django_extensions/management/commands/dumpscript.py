...
def get_models(app_labels):
    ...
    if not app_labels:
        for app in get_apps():
            models += [m for m in get_all_models(app) if m
    ...
    for app_label in app_labels:
        ...
        # Get all models for a given app
            ...
            models += [m for m in get_all_models(get_app(app_label))
...
class Code(object):
    ...
    def __str__(self):
        ...
        if self.imports:
            ...
            return flatten_blocks([''] + self.import_lines + ['']
                                  + self.lines, num_indents=self.indent)
        else:
    ...
    def get_import_lines(self):
        ...
        if self.imports:
            return ['from %s import %s' % (value, key) for (key,
                    value) in self.imports.items()]
        else:
            return []
...
class ModelCode(Code):
    ...
    def get_lines(self):
        ...
        for (counter, item) in enumerate(self.model.objects.all()):
            instance = InstanceCode(instance=item, id=counter + 1,
...
class InstanceCode(Code):
    ...
    def __init__(
        ...
        self.context = context
        self.variable_name = '%s_%s' % (self.instance._meta.db_table,
                id)
        self.skip_me = None
        ...
        for field in self.model._meta.many_to_many:
            self.many_to_many_waiting_list[field] = \
                list(getattr(self.instance, field.name).all())
    ...
    def skip(self):
        ...
        if reduce(lambda x, y: x + y, [self.model in so._meta.parents
    ...
    def instantiate(self):
        ...
        if not self.instantiated:
            code_lines.append('%s = %s()' % (self.variable_name,
                              self.model.__name__))
    ...
    def get_waiting_list(self, force=False):
        ...
        for field in list(self.waiting_list):
            try:
                ...
                value = get_attribute_value(self.instance, field,
                        self.context, force=force)
                code_lines.append('%s.%s = %s' % (self.variable_name,
                                  field.name, value))
    ...
    def get_many_to_many_lines(self, force=False):
        ...
        for (field, rel_items) in \
            self.many_to_many_waiting_list.items():
            for rel_item in list(rel_items):
                try:
                    ...
                    lines.append('%s.%s.add(%s)' % (self.variable_name,
                                 field.name, value))
                    self.many_to_many_waiting_list[field].remove(rel_item)
                except KeyError:
                    if force:
                        value = '%s.objects.get(%s=%s)' \
                            % (rel_item._meta.object_name, pk_name,
                               getattr(rel_item, pk_name))
                        lines.append('%s.%s.add(%s)'
                                % (self.variable_name, field.name,
                                value))
                        self.many_to_many_waiting_list[field].remove(rel_item)
...
class Script(Code):
    ...
    def get_lines(self):
        ...
        for model_class in queue_models(self.models,
                context=self.context):
        ...
        for model in self.models:
            ...
            for instance in model.instances:
                if instance.waiting_list \
                    or instance.many_to_many_waiting_list:
...
def get_attribute_value(
    ...
    # ForeignKey fields, link directly using our stored python variable name
        ...
        key = '%s_%s' % (value.__class__.__name__, getattr(value,
                         pk_name))
        ...
        if key in context:
            ...
            return '%s.objects.get(%s=%s)' % (value._meta.object_name,
                    pk_name, getattr(value, pk_name))
        else:
