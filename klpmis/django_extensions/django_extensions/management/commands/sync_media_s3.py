...
class Command(BaseCommand):
    ...
    option_list = BaseCommand.option_list + (
        optparse.make_option('-p', '--prefix', dest='prefix', default=''
                             ,
                             help='The prefix to prepend to the path on S3.'
                             ),
        optparse.make_option('-d', '--dir', dest='dir',
                             default=settings.MEDIA_ROOT,
                             help='The root directory to use instead of your MEDIA_ROOT'
                             ),
        optparse.make_option('--gzip', action='store_true', dest='gzip'
                             , default=False,
                             help='Enables gzipping CSS and Javascript files.'
                             ),
        optparse.make_option('--expires', action='store_true',
                             dest='expires', default=False,
    ...
    def handle(self, *args, **options):
        ...
        if not hasattr(settings, 'AWS_ACCESS_KEY_ID') \
            or not hasattr(settings, 'AWS_SECRET_ACCESS_KEY'):
        ...
        self.FILTER_LIST = getattr(settings, 'FILTER_LIST',
                                   self.FILTER_LIST)
    ...
    def sync_s3(self):
        ...
        os.path.walk(self.DIRECTORY, self.upload_s3, (bucket, key,
    ...
    def upload_s3(
        ...
        for file in names:
            ...
            if not self.do_force:
                ...
                if s3_key:
                    s3_datetime = \
                        datetime.datetime(*time.strptime(s3_key.last_modified,
                            '%a, %d %b %Y %H:%M:%S %Z')[0:6])
                    local_datetime = \
                        datetime.datetime.utcfromtimestamp(os.stat(filename).st_mtime)
                    if local_datetime < s3_datetime:
                        self.skip_count += 1
                        if self.verbosity > 1:
                            print "File %s hasn't been modified since last being uploaded" \
                                % file_key
                        continue
            ...
            if self.do_gzip:
                ...
                if file_size > 1024 and content_type \
                    in self.GZIP_CONTENT_TYPES:
                    filedata = self.compress_string(filedata)
                    headers['Content-Encoding'] = 'gzip'
                    if self.verbosity > 1:
                        print '\tgzipped: %dk to %dk' % (file_size
                                / 1024, len(filedata) / 1024)
            if self.do_expires:

                # HTTP/1.0

                headers['Expires'] = '%s GMT' \
                    % email.Utils.formatdate(time.mktime((datetime.datetime.now()
                        + datetime.timedelta(days=365
                        * 2)).timetuple()))

                # HTTP/1.1

                headers['Cache-Control'] = 'max-age %d' % (3600 * 24
                        * 365 * 2)
                if self.verbosity > 1:
            ...
            try:
                key.name = file_key
                key.set_contents_from_string(filedata, headers,
                        replace=True)
...
if not [opt for opt in Command.option_list if opt.dest == 'verbosity']:
    Command.option_list += (optparse.make_option(
        '-v',
        '--verbosity',
        dest='verbosity',
        default=1,
