...
def generate_dot(app_labels, **kwargs):
    ...
    for app in apps:
        graph = Context({
            ...
            'cluster_app_name': 'cluster_%s' % app.__name__.replace('.'
                    , '_'),
        ...
        for appmodel in get_models(app):
            abstracts = [e.__name__ for e in appmodel.__bases__
            ...
            def consider(model_name):
                return not include_models or model_name \
                    in include_models
            ...
            def add_relation(field, extras=''):
                _rel = {
                    'target_app': field.rel.to.__module__.replace('.',
                            '_'),
            ...
            if appmodel._meta.many_to_many:
                for field in appmodel._meta.many_to_many:
                    if isinstance(field, ManyToManyField) \
                        and getattr(field, 'creates_table', False):
                        add_relation(field,
                                '[arrowhead=normal arrowtail=normal]')
                    elif isinstance(field, GenericRelation):
                        add_relation(field,
                                mark_safe('[style="dotted"] [arrowhead=normal arrowtail=normal]'
                                ))
...
def main():
    try:
        (opts, args) = getopt.getopt(sys.argv[1:], 'hadgi:', ['help',
                'all_applications', 'disable_fields', 'group_models',
                'include_models='])
