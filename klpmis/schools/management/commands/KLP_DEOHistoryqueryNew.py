            ...
            if 1:
                ...
                for i in userIdsList:
                    if not i == None:
                        userIds.append(i)
                userIds = [111,112]
                asmIdsList = list(set(Answer.objects.filter(last_modified_date__range=(sTime, eTime)).values_list('question__assessment', flat=True)))
                assessments = Assessment.objects.filter(active=2, id__in = asmIdsList).distinct().only("id", "name")
                cursor = connection.cursor()

                stSql = """select id from fullhistory_fullhistory where action_time > '%s'  order by id limit 1 """ %(sTime)
                cursor.execute(stSql)
                res = cursor.fetchone()
                startId = res[0]

                etSql = """ select id from fullhistory_fullhistory where action_time < '%s'  order by id desc limit 1 """ %(eTime)
                cursor.execute(etSql)
                res = cursor.fetchone()
                endId = res[0]

                fullhistoryAnswers = \
                    cursor.execute("""select distinct CAST(object_id AS INT) from fullhistory_fullhistory where id > %s and id < %s and content_type_id in ( select id from django_content_type where name='%s')"""  % (startId, endId,'answer'))
                validAns = "0 "
                ...
                if users:
                    for user in users:
                        ...
                        for content in contentList:
                            ...
                            cursor.execute("select count(id) from fullhistory_fullhistory where id > %s and id < %s and request_id in ( select id from fullhistory_request where user_pk = %s) and content_type_id in ( select id from django_content_type where id=%s) and CAST(object_id AS INT) in %s and action=%s",[startId, endId, userId, contId, tuple(preList),'C'])
                            res = cursor.fetchone()
                            dataList.append(int(res[0]))

                            cursor.execute("select count(id) as count from fullhistory_fullhistory where id > %s and id < %s and request_id in (select id from fullhistory_request where user_pk =%s ) and content_type_id in ( select id from django_content_type where id=%s) and CAST(object_id AS INT)  in %s and action=%s and data ~* %s",[startId, endId, userId, contId, tuple(preList),'U','active'])
                            res = cursor.fetchone()
                            dataList.append(int(res[0]))

                            cursor.execute("select count(id) as count from fullhistory_fullhistory where id > %s and id < %s and request_id in (select id from fullhistory_request where user_pk =%s ) and content_type_id in ( select id from django_content_type where id=%s) and CAST(object_id AS INT)  in %s and action=%s and not data  ~* %s ",[startId, endId, userId, contId, tuple(preList),'U','active'])
                            res = cursor.fetchone()
                            dataList.append(int(res[0]))
                        ...
                        for asmId in asmList:
                            ...
                            if answers:
                                ...
                                crQuery = """select count(id) as count from fullhistory_fullhistory where id > %s and id < %s and request_id  in (select id from fullhistory_request where user_pk =%s )  and CAST(object_id AS INT)  in %s and action='C'""" % (startId, endId, userId, tuple(answers))
                                cursor.execute(crQuery)
                                res = cursor.fetchone()
                                crEntries = int(res[0])

                                # ids of answer records that i created for an assessment...
                                crQueryData = """select object_id as count from fullhistory_fullhistory where id > %s and id < %s and request_id in (select id from fullhistory_request where user_pk =%s )  and CAST(object_id AS INT)  in %s and action='C'""" % (startId, endId, userId, tuple(answers))
                                ...
                                if crEntries == 00:
                                    ...
                                    cursor.execute("select count(id) from fullhistory_fullhistory where id > %s and id < %s and action = 'U' and not request_id in ( select id from fullhistory_request where user_pk = %s) and CAST(object_id AS INT) in %s and data ~* %s and (data ~* %s or data ~* %s ) and not (data ~* %s or data ~* %s or data ~* %s)",[startId, endId, userId, tuple(crEntriesLisInt),'user2','answer','status','id','question','student'])
                                    res = cursor.fetchone()
                                    inCrEntries = int(res[0])

                                # verified entries sql
                                # how many others records I verified by double entry for an assessment...
                                cursor.execute("select count(id) from fullhistory_fullhistory where id > %s and id < %s and action = 'U' and request_id in ( select id from fullhistory_request where user_pk = %s) and CAST(object_id AS INT) in %s and data ~* %s and not (data ~* %s or data ~* %s or data ~* %s or data ~* %s or data ~* %s)",[startId, endId, userId, tuple(answers),'user2','id','question','student','answer', 'status'])
                                res = cursor.fetchone()
                                vEntries = int(res[0])

                                # rectfied entires sql
                                # how many others wrong answers I corrected/ fixed by double entry for an assessment...
                                cursor.execute("select count(id) from fullhistory_fullhistory where id > %s and id < %s and action = 'U' and request_id in ( select id from fullhistory_request where user_pk = %s) and CAST(object_id AS INT) in %s and data ~* %s and (data ~* %s or data ~* %s ) and not (data ~* %s or data ~* %s or data ~* %s)",[startId, endId, userId, tuple(answers),'user2','answer','status','id','question','student'])
                                res = cursor.fetchone()
                                rEntries = int(res[0])
