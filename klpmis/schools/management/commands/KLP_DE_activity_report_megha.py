            ...
            if fileName and startDate and endDate:
                if 1: #try:
                    ...
                    for assessmentId in assessmentsInvolvedInReport:
                        assessmentTitle = Assessment.objects.get(id=assessmentId).name
			#programId = Assessment.objects.get(id=assessmentId).programme
			programmeTitle = str(Assessment.objects.get(id=assessmentId).programme)
			#programmeTitle = Programme.objects.get(id=programId).name
                        headerList.append(programmeTitle + '_' + assessmentTitle+'_created')
                        headerList.append(programmeTitle + '_' + assessmentTitle+'_verified')
                        headerList.append(programmeTitle + '_' + assessmentTitle+'_rectified')
                        headerList.append(programmeTitle + '_' + assessmentTitle+'_wrong')
                    ...
                    for i,user in enumerate(users):
                        ...
                        if answersUpdatedByUser:
                            for assessmentId in assessmentsInvolvedInReport:
                            	...
                            	answer_wrong =  validFullHistoryRecords.filter( content_type__id=contentTypeIds['answer'], object_id__in=answersUpdatedForGivenAssessment, action='U', _data__contains='modifiedBy": ['+str(user.id)+', ', _data__icontains='answer').exclude(request__user_pk=user.id).count()     #How many of my answers were wrong...; showing all recored, which were created by me, but now updated by others.
