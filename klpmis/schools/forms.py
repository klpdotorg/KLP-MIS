...
class Institution_address_Form(ModelForm):
    ...
    {'rows': 4, 'cols': 20}))
    area = forms.CharField(widget=forms.Textarea(attrs=
    {'rows': 4, 'cols': 20}), required=False)
    pincode = forms.CharField(required=False)
    landmark = forms.CharField(widget=forms.Textarea
    (attrs={'rows': 4, 'cols': 20}), required=False)
...
class Programme_Form(ModelForm):
    ...
    (format = '%d-%m-%Y'), initial = default_end_date,
    input_formats = ['%d-%m-%Y', '%d-%m-%y'])
    active = forms.IntegerField(initial=2, widget=forms.HiddenInput)
...
class Assessment_Form(ModelForm):
    ...
    input_formats=['%d-%m-%Y', '%d-%m-%y'])
    endDate = forms.DateField(widget=forms.DateInput
    (format='%d-%m-%Y'), initial = default_end_date, input_formats=
    ['%d-%m-%Y', '%d-%m-%y'])
    active = forms.IntegerField(initial=2, widget=forms.HiddenInput)
...
class Question_Form(ModelForm):
    ...
    scoreMin = forms.DecimalField(max_digits=5, decimal_places=2,
    required=False)
    scoreMax = forms.DecimalField(max_digits=5, decimal_places=2,
    required=False)
        ...
        if questionType == '1':
            if not scoreMax and scoreMax != 0:
                ...
                if scoreMin > scoreMax:
                    ...
                    ("Score Max Should be Grater than Score Min.")
        return scoreMax
